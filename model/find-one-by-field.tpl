...
{{if .withCache}}{{.cacheKey}}
	...
	if err := conn.QueryRowCtx(ctx, &resp, query, {{.lowerStartCamelField}},bconst.StatusDeleteNo); err != nil {
		return nil, err
	}
	...
	query := fmt.Sprintf("select %s from %s where {{.originalField}}  and deleted = ? limit 1", {{.lowerStartCamelObject}}Rows, m.table )
	err := m.conn.QueryRowCtx(ctx, &resp, query, {{.lowerStartCamelField}},bconst.StatusDeleteNo)
	switch err {
	case nil:
		return &resp, nil
	case sqlc.ErrNotFound:
